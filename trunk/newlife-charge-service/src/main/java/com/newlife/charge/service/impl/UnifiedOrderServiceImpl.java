    ...
    public String createOrder(CreateOrderIn queryIn, Integer userId, String type) {
        ...
        try {
            ...
            if(queryIn.getCouId() !=null && queryIn.getCuId() != null){
                ...
                if(userCoupon.getStatus() == UserCouponStatusEnum.UNUSED.getValue() && // 使用状态必须是未使用
                        coupon.getStatus() == CouponStatusEnum.USED.getValue() && // 优惠券没有下架
                        coupon.getUseEndTime().getTime() >= date.getTime() && // 优惠券没有过期
                        coupon.getType() == CouponTypeEnum.Charge_Type.getValue() && // 优惠券是充值优惠券

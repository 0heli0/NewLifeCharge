...
public class CouponServiceImpl implements CouponService {
    ...
    public int save(CouponSaveIn saveIn) {
        ...
        if(saveIn != null){
            ...
            if(saveIn.getType() == CouponTypeEnum.Electro_Type.getValue() && saveIn.getStationIds().size()>0){
            ...
            if(coupon.getUseEndTime().getTime() > new Date().getTime()){
...
//            couponInfo.setScopeType(coupon.getScopeType());
            ...
            if(saveIn.getType() == CouponTypeEnum.Electro_Type.getValue()){
    ...
    public CouponOut info(int id) {
        ...
        if(info != null){
            ...
            // 判断是否已下架
            if(out.getCollectEndTime().getTime() <= new Date().getTime()){
                out.setIsSoldOut(1);
            }else {
                out.setIsSoldOut(0);
            }
...
//            couponInfo.setScopeType(CouponScopeTypeEnum.All_Scope.getValue());
            ...
            if(updateIn.getType() == CouponTypeEnum.Electro_Type.getValue()){
    ...
    public CouponOut stationInfo(int id) {
        ...
        if(Collections3.isNotEmpty(userCoupons)) {
            List<UserCoupon> userCount = userCoupons.stream().filter(userCoupon -> UserCouponStatusEnum.USED.getValue() == userCoupon.getStatus()).collect(Collectors.toList());
    ...
    private boolean checkPricIsRepeat(Coupon couponIn, List<Integer> stationIds){
        ...
        // 通用优惠券存在包含关系，有则判断为重复
        if(couponIn.getScopeType() == CouponScopeTypeEnum.All_Scope.getValue()){
            throw new BizException(ERROR.COUPON_REPEAT);
        }
        // 查看指定券是否存在包含关系
